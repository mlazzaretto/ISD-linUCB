...
class D_linUCB:
    ...
    Russac, Yoan, Claire Vernade, and Olivier Cappé. "Weighted linear bandits
    for non-stationary environments." Advances in Neural Information
    Processing Systems 32 (2019).
    '''
    def __init__(self, p, K, gamma_0, Sigma, lmbd, delta, g, rng):
        ...
        self.g2_t = 1           # squared cum. prod. of discounting factor
        ...
        self.alpha = self.m2 * np.sqrt(lmbd) + \
            np.sqrt(2 * np.log(1 / delta) +
                    self.p *
                    np.log(1 + (1-self.g2_t) /
                           (self.p * self.lmbd*(1 - self.g**2))))
    ...
    def update(self, gamma_0_t=False, Xa_t=False, Sigma=False):
        ...
        self.Sigma = self.g * self.Sigma + np.outer(X_opt, X_opt.T) \
            + (1-self.g) * self.lmbd * np.eye(self.p)
        self.cov = self.g * self.cov + self.R[-1] * X_opt
